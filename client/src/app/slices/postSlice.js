    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(reactToPost.fulfilled, (state, action) => {
                ...
                const uIdx = state.userPosts.findIndex(p => p._id === postId);
                if (uIdx > -1) {
                    state.userPosts[uIdx].reactions = reactions;
                    state.userPosts[uIdx].reactionCount = count;
                    state.userPosts[uIdx].userReaction = userReaction;
                }
            })
            .addCase(addComment.fulfilled, (state, action) => {
                ...
                const uPost = state.userPosts.find(p => p._id === postId);
                if (uPost) {
                    uPost.comments = [...(uPost.comments || []), comment];
                }
            })
