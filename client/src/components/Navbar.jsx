        ...
        <nav className="sticky top-0 left-0 right-0 h-[72px] z-10001 bg-bg-dark/95 backdrop-blur-xl border-b border-white/10 px-6 flex items-center">
            <div className="flex items-center justify-between w-full max-w-[1440px] mx-auto">
                <div className="flex items-center gap-6">
                    ...
                    <button 
                        className="lg:hidden flex items-center justify-center w-10 h-10 -ml-2 rounded-xl hover:bg-white/5 transition-all duration-200 cursor-pointer z-1100" 
                        onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
                        aria-label="Toggle menu"
                    >
                        <div className="relative w-6 h-5 flex flex-col justify-between items-center py-1">
                            <span className={`block w-5 h-0.5 bg-text-primary rounded-full transition-all duration-300 ${mobileMenuOpen ? 'translate-y-1.5 rotate-45' : ''}`}></span>
                            <span className={`block w-5 h-0.5 bg-text-primary rounded-full transition-all duration-300 ${mobileMenuOpen ? 'opacity-0 scale-x-0' : ''}`}></span>
                            <span className={`block w-5 h-0.5 bg-text-primary rounded-full transition-all duration-300 ${mobileMenuOpen ? '-translate-y-1.5 -rotate-45' : ''}`}></span>
                        </div>
                    </button>
                ...
                <div className="flex items-center gap-1.5 sm:gap-4 ml-auto">
                    {user ? (
                        <>
                            ...
                            <button onClick={() => window.dispatchEvent(new CustomEvent('toggle-chat'))} className="relative p-2 sm:p-2.5 rounded-xl text-text-secondary hover:text-text-primary border border-white/10 transition-all duration-200 flex cursor-pointer">
                                <MessageSquare size={18} />
                            </button>

                            <button onClick={() => window.dispatchEvent(new CustomEvent('toggle-notifications'))} className="relative p-2 sm:p-2.5 rounded-xl text-text-secondary hover:text-text-primary border border-white/10 transition-all duration-200 flex cursor-pointer">
                                <Bell size={18} />
            ...
            {/* Mobile Sidebar Menu */}
            <div className={`fixed inset-0 bg-black/80 backdrop-blur-md z-9998 transition-all duration-500 ${mobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`} onClick={() => setMobileMenuOpen(false)}>
                <div className={`fixed top-0 left-0 w-[280px] h-full bg-bg-dark border-r border-white/10 flex flex-col z-9999 shadow-2xl transition-transform duration-400 cubic-bezier(0.4, 0, 0.2, 1) ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`} onClick={e => e.stopPropagation()}>
                    <div className="p-6 flex items-center justify-between border-b border-white/5 bg-white/2">
                        <div className="flex items-center gap-3">
                            <div className="w-9 h-9 bg-white rounded-lg flex items-center justify-center text-indigo-600 shadow-[0_0_15px_rgba(99,102,241,0.3)]"><Zap size={20} fill="currentColor" /></div>
                            <span className="font-display font-bold text-xl tracking-tight text-text-primary">Career<span className="text-primary-light">Lens</span></span>
                        </div>
                        <button className="w-9 h-9 flex items-center justify-center text-text-secondary hover:text-white hover:bg-white/10 rounded-xl transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>
                            <X size={20} />
                        </button>
                    </div>

                    <div className="p-5 border-b border-white/3">
                        <div className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/5 border border-white/10 focus-within:bg-white/10 focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/10 transition-all duration-300 group">
                            <Search size={16} className="text-text-muted group-focus-within:text-primary-light transition-colors" />
                            <input
                                type="text"
                                placeholder="Search everything..."
                                className="w-full bg-transparent border-none outline-none text-[15px] text-text-primary placeholder:text-text-muted font-medium"
                                value={searchInput}
                                onChange={e => {
                    ...
                    <div className="flex-1 px-4 py-6 flex flex-col gap-1.5 overflow-y-auto custom-scrollbar">
                        {links.map(l => (
                            <Link 
                                key={l.path} 
                                to={l.path} 
                                className={`flex items-center gap-4 px-4 py-3.5 rounded-2xl text-[15px] font-semibold transition-all duration-200 ${
                                    isActive(l.path) 
                                    ? 'bg-primary/10 text-primary-light ring-1 ring-primary/20 shadow-inner' 
                                    : 'text-text-secondary hover:bg-white/5 hover:text-text-primary'
                                }`}
                                onClick={() => setMobileMenuOpen(false)}
                            >
                                <span className={`${isActive(l.path) ? 'text-primary' : 'text-text-muted'} transition-colors`}>{l.icon}</span>
                                <span>{l.label}</span>
                            </Link>
                        ))}
                    </div>
                    <div className="p-6 border-t border-white/10 bg-white/1">
                        {user ? (
                            <div className="space-y-3">
                                <Link to="/me" className="w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-2xl bg-white/5 border border-white/10 text-text-secondary text-[15px] font-bold hover:text-white hover:bg-white/10 transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>
                                    <User size={18} /> My Profile
                                </Link>
                                <button className="w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-2xl bg-red-400/10 text-red-400 border border-red-400/20 font-bold hover:bg-red-400/20 transition-all duration-200 cursor-pointer" onClick={handleLogout}>
                                    <LogOut size={16} /> Sign Out
                                </button>
                            </div>
                        ) : (
                            <div className="grid grid-cols-2 gap-3">
                                <Link to="/login" className="flex items-center justify-center py-3.5 rounded-2xl border border-white/10 text-sm font-bold text-text-secondary hover:text-white hover:bg-white/5 transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>Sign In</Link>
                                <Link to="/signup" className="flex items-center justify-center py-3.5 rounded-2xl bg-primary text-white text-sm font-bold shadow-lg shadow-primary/20 hover:brightness-110 active:scale-95 transition-all duration-200" onClick={() => setMobileMenuOpen(false)}>Get Started</Link>
                            </div>
                        )}
                    </div>
                </div>
