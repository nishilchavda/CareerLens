      ...
      <div className="relative z-10 w-full max-w-5/6 grid grid-cols-1 lg:grid-cols-2 bg-bg-card/40 backdrop-blur-3xl border border-white/10 rounded-4xl shadow-2xl overflow-hidden animate-scale min-h-[750px]">
              ...
              {[
                {
                  ...
                  desc: "Instant score & actionable feedback",
                  color: "text-primary",
                },
                {
                  icon: Zap,
                  label: "Smart Job Matching",
                  desc: "AI ranks jobs by your unique profile fit",
                  color: "text-secondary",
                },
                {
                  icon: Target,
                  label: "Verified Rankings",
                  desc: "Get prioritized in high-quality talent pools",
                  color: "text-amber-400",
                },
              ].map((feat, i) => (
                <div key={i} className="flex gap-4 group">
                  <div
                    className={`w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center ${feat.color} group-hover:scale-110 transition-transform`}
                  >
                    <feat.icon size={20} />
                  </div>
                  <div>
          ...
          <div className="pt-8 border-t border-white/5 flex items-center justify-between">
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className="w-10 h-10 rounded-full border-2 border-bg-dark bg-white/10 overflow-hidden"
                >
            ...
            <div className="text-sm font-medium text-text-muted">
              <span className="text-text-primary font-bold">50K+</span> pros
              already joined
            </div>
          </div>
        </div>
        ...
        <div className="p-8 lg:p-14 flex flex-col justify-center overflow-y-auto">
          ...
          <p className="mt-8 text-center text-text-muted text-xs font-medium">
            ...
            <Link
              to="/login"
              className="text-text-primary font-black hover:underline ml-1"
            >
              Sign in
            </Link>
          </p>
